@php
$id = $id ?? 1;
$bulan = urldecode($bulan ?? 'Agustus 2025'); // <-- decode URL
...
$allBerita = [
    'Agustus 2025' => array_map(fn($i) => [
        ...
        'content' => "Konten lengkap Berita Agustus $i",
    ...
    'Juli 2025' => array_map(fn($i) => [
        ...
        'content' => "Konten lengkap Berita Juli $i",
        'date'  => "28 Juli 2025",
        'author'=> "Admin SMP Wahidiyah",
        'image' => 'images/beritacontoh.png',
    ], range(1,10)),
    'Juni 2025' => array_map(fn($i) => [
        ...
        'content' => "Konten lengkap Berita Juni $i",
        'date'  => "28 Juni 2025",
        'author'=> "Admin SMP Wahidiyah",
        'image' => 'images/beritacontoh.png',
    ], range(1,10)),
...
// Ambil berita sesuai bulan & id
$berita = $allBerita[$bulan][$id-1] ?? null;

// Flatten semua berita untuk Berita Lainnya
$flatBerita = [];
foreach ($allBerita as $bulanKey => $monthBerita) {
    foreach ($monthBerita as $item) {
        $item['bulan'] = $bulanKey; // simpan info bulan
        $flatBerita[] = $item;
    }
}
    ...
    <div class="space-y-6 mb-10">
        @foreach ($beritaLainnya as $item)
            <div class="bg-white shadow-lg rounded-xl overflow-hidden flex flex-col md:flex-row">
                <a href="{{ url('/berita/detail/'.urlencode($item['bulan']).'/'.($loop->iteration)) }}" class="md:w-1/3 w-full">
                    <img src="{{ asset($item['image']) }}" alt="{{ $item['title'] }}"
                         class="w-full h-full object-cover rounded-l-xl md:rounded-l-none md:rounded-t-xl md:h-40">
                </a>
                <div class="p-4 flex-1">
                    <a href="{{ url('/berita/detail/'.urlencode($item['bulan']).'/'.($loop->iteration)) }}">
